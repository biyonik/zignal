import { Component, forwardRef } from '@angular/core';
import { CommonModule } from '@angular/common';
import { NG_VALUE_ACCESSOR, NG_VALIDATORS } from '@angular/forms';
import { BaseNativeComponent } from './base-native.component';
import { MoneyField } from '../../../fields/money.field';

@Component({
    selector: 'zg-money',
    standalone: true,
    imports: [CommonModule],
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => ZgMoneyComponent),
            multi: true,
        },
        {
            provide: NG_VALIDATORS,
            useExisting: forwardRef(() => ZgMoneyComponent),
            multi: true,
        },
    ],
    template: `
        <div class="zg-field zg-money-field" [class]="cssClass()">
            <label *ngIf="field().label" [for]="field().name" class="zg-label">
                {{ field().label }}
                <span *ngIf="field().config.required" class="zg-required">*</span>
            </label>

            <div class="zg-money-wrapper">
                <span class="zg-money-symbol">{{ currencySymbol }}</span>
                <input
                    type="number"
                    [id]="field().name"
                    [name]="field().name"
                    [value]="value"
                    [placeholder]="field().config.placeholder ?? '0.00'"
                    [disabled]="disabledStatus"
                    [readonly]="readonly()"
                    [min]="field().config.min"
                    [max]="field().config.max"
                    [step]="step"
                    [attr.aria-label]="field().label"
                    [attr.aria-invalid]="showError"
                    [class.zg-invalid]="showError"
                    class="zg-input"
                    (input)="onInput($event)"
                    (blur)="handleBlur()"
                />
            </div>

            <small *ngIf="field().config.hint && !showError" class="zg-hint">
                {{ field().config.hint }}
            </small>

            <small *ngIf="showError" class="zg-error">
                {{ error }}
            </small>
        </div>
    `,
    styles: [`
        .zg-field {
            display: flex;
            flex-direction: column;
            gap: 4px;
            margin-bottom: 16px;
        }
        .zg-label { font-weight: 500; font-size: 14px; }
        .zg-required { color: #ef4444; }
        .zg-money-wrapper {
            display: flex;
            align-items: center;
        }
        .zg-money-symbol {
            padding: 8px 12px;
            background: #f3f4f6;
            border: 1px solid #d1d5db;
            border-right: none;
            border-radius: 6px 0 0 6px;
            font-size: 14px;
            color: #6b7280;
        }
        .zg-input {
            flex: 1;
            padding: 8px 12px;
            border: 1px solid #d1d5db;
            border-radius: 0 6px 6px 0;
            font-size: 14px;
        }
        .zg-input:focus {
            outline: none;
            border-color: #3b82f6;
            box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
        }
        .zg-input.zg-invalid { border-color: #ef4444; }
        .zg-input:disabled { background-color: #f3f4f6; cursor: not-allowed; }
        .zg-hint { color: #6b7280; font-size: 12px; }
        .zg-error { color: #ef4444; font-size: 12px; }
    `],
})
export class ZgMoneyComponent extends BaseNativeComponent<MoneyField, unknown> {
    get currencySymbol(): string {
        const symbols: Record<string, string> = {
            TRY: '₺',
            USD: '$',
            EUR: '€',
            GBP: '£',
        };
        return symbols[this.field().config.defaultCurrency ?? 'TRY'] ?? this.field().config.defaultCurrency ?? '₺';
    }

    get step(): string {
        const decimals = this.field().config.decimals ?? 2;
        return (1 / Math.pow(10, decimals)).toString();
    }

    onInput(event: Event): void {
        const input = event.target as HTMLInputElement;
        const value = input.valueAsNumber;
        this.updateValue(isNaN(value) ? null as any : value);
    }
}
