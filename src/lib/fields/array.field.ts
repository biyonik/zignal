...
import { FieldConfig, FieldValue, IField, t } from '../core';
...
export class ArrayField extends BaseField<Record<string, unknown>[]> {
    ...
    schema(): z.ZodType<Record<string, unknown>[]> {
        ...
        if (this.config.min != null) {
            arraySchema = arraySchema.min(this.config.min, t('array.min', { min: this.config.min }));
        }
        if (this.config.max != null) {
            arraySchema = arraySchema.max(this.config.max, t('array.max', { max: this.config.max }));
        }
        if (this.config.required) {
            arraySchema = arraySchema.min(1, t('required'));
        }
