...
export class NumberField extends BaseField<number> {
  ...
  schema(): z.ZodType<number> {
    ...
    if (this.config.max !== undefined) {
      base = base.max(
        this.config.max,
        `En fazla ${this.config.max} olabilir`
      );
    }
