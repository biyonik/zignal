  ...
  schema(): z.ZodType<FileInfo | FileInfo[] | null> {
    ...
    if (this.config.accept?.length) {
      ...
      singleFileSchema = singleFileSchema.refine(
        ...
        { message: `İzin verilen dosya tipleri: ${this.getAcceptDescription()}` }
      );
    }
    ...
    if (this.config.extensions?.length) {
      ...
      singleFileSchema = singleFileSchema.refine(
        ...
        { message: `İzin verilen uzantılar: ${this.config.extensions.join(', ')}` }
      );
    }
    ...
    if (this.config.maxSize !== undefined) {
      ...
      singleFileSchema = singleFileSchema.refine(
        ...
        { message: `Dosya boyutu en fazla ${this.formatSize(maxSize)} olabilir` }
      );
    }
    ...
    if (this.config.minSize !== undefined) {
      ...
      singleFileSchema = singleFileSchema.refine(
        ...
        { message: `Dosya boyutu en az ${this.formatSize(minSize)} olmalıdır` }
      );
    }
    ...
    if (this.config.multiple) {
      ...
      if (this.config.maxFiles !== undefined) {
        arraySchema = z.array(singleFileSchema).max(
          ...
          `En fazla ${this.config.maxFiles} dosya seçilebilir`
        );
      }
